...
func (h *AccountHandler) List(c *gin.Context) {
	...
	sortOrder := strings.TrimSpace(c.DefaultQuery("sort_order", service.AccountListSortOrderDesc))
	if sortBy != "" && !service.IsAccountListSortBySupported(sortBy) {
		response.ErrorFrom(c, infraerrors.BadRequest("INVALID_ACCOUNT_SORT", "invalid account sort"))
		return
	}
	if !service.IsAccountListSortOrderSupported(sortOrder) {
		response.ErrorFrom(c, infraerrors.BadRequest("INVALID_ACCOUNT_SORT_ORDER", "invalid account sort order"))
		return
	}
	sortOrder = service.NormalizeAccountListSortOrder(sortOrder)
	// 标准化和验证 search 参数
...
func (h *AccountHandler) BatchRefreshTier(c *gin.Context) {
	...
	if len(req.AccountIDs) == 0 {
		allAccounts, _, err := h.adminService.ListAccounts(ctx, 1, 10000, "gemini", "oauth", "", "", 0, "", "", service.AccountListSortOrderDesc)
		if err != nil {
			response.ErrorFrom(c, err)
			return
		}
