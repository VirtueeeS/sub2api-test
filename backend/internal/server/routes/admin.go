...
func registerCPARoutes(admin *gin.RouterGroup, h *handler.Handlers) {
	cpaGroup := admin.Group("/cpa")
	{
		cpaGroup.GET("/overview", h.Admin.CPA.GetOverview)
		cpaGroup.PUT("/config", h.Admin.CPA.UpdateConfig)
		cpaGroup.POST("/run", h.Admin.CPA.RunNow)
		cpaGroup.GET("/jobs", h.Admin.CPA.ListJobs)
		cpaGroup.GET("/jobs/:job_id/records", h.Admin.CPA.ListJobRecords)
		cpaGroup.GET("/files", h.Admin.CPA.ListFileStates)
	}
}
