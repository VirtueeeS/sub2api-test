...
type AdminService interface {
	...
	// Account management
	ListAccounts(ctx context.Context, page, pageSize int, platform, accountType, status, search string, groupID int64, privacyMode string, sortArgs ...string) ([]Account, int64, error)
...
func (s *adminServiceImpl) ListAccounts(ctx context.Context, page, pageSize int, platform, accountType, status, search string, groupID int64, privacyMode string, sortArgs ...string) ([]Account, int64, error) {
	params := pagination.PaginationParams{Page: page, PageSize: pageSize}
	sortBy := ""
	sortOrder := AccountListSortOrderDesc
	if len(sortArgs) > 0 {
		sortBy = sortArgs[0]
	}
	if len(sortArgs) > 1 {
		sortOrder = NormalizeAccountListSortOrder(sortArgs[1])
	}
