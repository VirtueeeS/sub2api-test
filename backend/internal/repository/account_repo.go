...
func (r *accountRepository) ListWithFilters(ctx context.Context, params pagination.PaginationParams, platform, accountType, status, search string, groupID int64, privacyMode, sortBy, sortOrder string) ([]service.Account, *pagination.PaginationResult, error) {
	...
	if sortBy == service.AccountListSortByUsage {
		accounts, err := q.All(ctx)
		if err != nil {
			return nil, nil, err
		}
		usageTotals, err := r.listTodayUsageTotalsByAccountIDs(ctx, extractAccountIDs(accounts))
		if err != nil {
			return nil, nil, err
		}
	...
	if orderTerms := accountListOrderTerms(sortBy, sortOrder); len(orderTerms) > 0 {
		query = query.Order(orderTerms...)
	} else {
		query = query.Order(dbent.Desc(dbaccount.FieldID))
	}
...
func extractAccountIDs(accounts []*dbent.Account) []int64 {
...
func sortAccountsByUsageTotals(accounts []*dbent.Account, usageTotals map[int64]float64, sortOrder string) {
	descending := service.NormalizeAccountListSortOrder(sortOrder) != service.AccountListSortOrderAsc
	sort.Slice(accounts, func(i, j int) bool {
		left := usageTotals[accounts[i].ID]
		right := usageTotals[accounts[j].ID]
		if left == right {
			if descending {
...
func accountListOrderTerms(sortBy, sortOrder string) []dbaccount.OrderOption {
	...
	case service.AccountListSortByStatus:
		field = dbaccount.FieldStatus
	case service.AccountListSortBySchedulable:
		field = dbaccount.FieldSchedulable
	case service.AccountListSortByPriority:
		field = dbaccount.FieldPriority
	case service.AccountListSortByRateMultiplier:
		field = dbaccount.FieldRateMultiplier
	case service.AccountListSortByLastUsedAt:
		field = dbaccount.FieldLastUsedAt
	case service.AccountListSortByExpiresAt:
...
func (r *accountRepository) listTodayUsageTotalsByAccountIDs(ctx context.Context, ids []int64) (map[int64]float64, error) {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&accountID, &usageTotal); err != nil {
			return nil, err
		}
		result[accountID] = usageTotal
	}
	if err := rows.Err(); err != nil {
		return nil, err
	}
