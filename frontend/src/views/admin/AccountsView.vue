...
const ACCOUNT_SERVER_SORT_KEYS: AccountSortKey[] = [
  'name',
  'status',
  'schedulable',
  'priority',
...
const ACCOUNT_SERVER_SORT_KEY_SET = new Set<string>(ACCOUNT_SERVER_SORT_KEYS)
const DEFAULT_ACCOUNT_SORT_BY: AccountSortKey = 'name'
const DEFAULT_ACCOUNT_SORT_ORDER: AccountSortOrder = 'asc'

const normalizeAccountSortBy = (candidate: unknown): AccountSortKey => {
  if (typeof candidate === 'string' && ACCOUNT_SERVER_SORT_KEY_SET.has(candidate)) {
    return candidate as AccountSortKey
  }
  return DEFAULT_ACCOUNT_SORT_BY
}
...
const persistAccountSort = (sortBy: AccountSortKey, sortOrder: AccountSortOrder) => {
  if (typeof window === 'undefined') return
  try {
    localStorage.setItem(ACCOUNT_SORT_STORAGE_KEY, JSON.stringify({ key: sortBy, order: sortOrder }))
  } catch (e) {
    console.error('Failed to save account sort settings:', e)
  }
}

const loadSavedAccountSort = (): { sortBy: AccountSortKey; sortOrder: AccountSortOrder } => {
  if (typeof window === 'undefined') {
    return { sortBy: DEFAULT_ACCOUNT_SORT_BY, sortOrder: DEFAULT_ACCOUNT_SORT_ORDER }
  }
  try {
    ...
    return {
      sortBy: normalizeAccountSortBy(parsed.key),
      sortOrder: normalizeAccountSortOrder(parsed.order)
    }
  } catch (e) {
...
let initialAccountSort: { sortBy: AccountSortKey; sortOrder: AccountSortOrder } = {
  sortBy: DEFAULT_ACCOUNT_SORT_BY,
  sortOrder: DEFAULT_ACCOUNT_SORT_ORDER
}
...
if (typeof window !== 'undefined') {
  ...
  const savedAccountSort = loadSavedAccountSort()
  if (savedAccountSort.sortBy !== DEFAULT_ACCOUNT_SORT_BY && hiddenColumns.has(savedAccountSort.sortBy)) {
    persistAccountSort(DEFAULT_ACCOUNT_SORT_BY, DEFAULT_ACCOUNT_SORT_ORDER)
  } else {
    initialAccountSort = savedAccountSort
  }
}
...
const toggleColumn = (key: string) => {
  ...
  if (!wasHidden && params.sort_by === key) {
    params.sort_by = DEFAULT_ACCOUNT_SORT_BY
    params.sort_order = DEFAULT_ACCOUNT_SORT_ORDER
    persistAccountSort(DEFAULT_ACCOUNT_SORT_BY, DEFAULT_ACCOUNT_SORT_ORDER)
    reload().catch((error) => {
      console.error('Failed to reload accounts after resetting hidden column sort:', error)
    })
  }
...
const {
  ...
  initialParams: {
    ...
    group: '',
    search: '',
    sort_by: initialAccountSort.sortBy,
    sort_order: initialAccountSort.sortOrder
  }
})
...
const handleSort = (key: string, order: AccountSortOrder) => {
  params.sort_by = normalizeAccountSortBy(key)
